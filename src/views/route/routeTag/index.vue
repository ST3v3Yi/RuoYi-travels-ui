    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['route:routeTag:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['route:routeTag:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['route:routeTag:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listRouteTag, getRouteTag, delRouteTag, addRouteTag, updateRouteTag } from "@/api/route/routeTag";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listRouteTag(this.queryParams).then(response => {
        this.routeTagList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateRouteTag(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addRouteTag(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除路线Tag编号为"' + ids + '"的数据项？').then(function() {
        return delRouteTag(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('route/routeTag/export', {
        ...this.queryParams
      }, `routeTag_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
