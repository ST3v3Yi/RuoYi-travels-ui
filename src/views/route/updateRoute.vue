...
import {uploadCoverImg, addRoute, getRoute, updateRoute} from "@/api/route/route";
...
export default {
  ...
  methods: {
    getRoute() {
      getRoute(this.$route.query.id).then((res) => {
        this.title = res.data.title;
        this.image = process.env.VUE_APP_BASE_API + res.data.coverImg;
        this.route.coverImg = res.data.coverImg;
        this.infoForm.day = res.data.day;
        this.infoForm.cost = res.data.cost;
    ...
    save() {
      this.route.id = this.$route.query.id;
      // 获取路线标题
      ...
      this.route.isDeleted = 1;
      updateRoute(this.route).then((res) => {
        this.$notify({
          title: '成功',
          message: '成功保存路线攻略~',
          type: 'success'
